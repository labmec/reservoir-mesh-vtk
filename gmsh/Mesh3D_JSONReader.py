...
def read_json(filename):
    ...
    for key, value in data.items():
        if key == "WellboreData":
            wellbore_data = value
        elif key == "ReservoirData":
            reservoir_data = value

    return wellbore_data, reservoir_data

wellbore_data, reservoir_data = read_json(filename)
...
if __name__ == "__main__":
    ...
    # ===============================================================================
    # WELLBORE
    lw = wellbore_data['length'] # length (m) of the wellbore
    Rw= wellbore_data['radius'] # radius (m) of the wellbore
    ...
    lr = reservoir_data['length'] # length (m) of the reservoir
    Wr = reservoir_data['width'] # width (m) of the reservoir
    Hr = reservoir_data['height']  # height (m) of the reservoir
    ...
    ecc = wellbore_data['eccentricity'][2] 
    ...
    # ===============================================================================
    # NÃO ASSUMINDO TAGS DEPOIS DA EXTRUSÃO
    # ===============================================================================
    
    new_points = [entity for entity in ids if entity[0] == 0]
    new_lines = [entity for entity in ids if entity[0] == 1]
    new_surfaces = [entity for entity in ids if entity[0] == 2]
    ...
    # ===============================================================================
    # COPIAS INTERESSANTES PARA O TESTE DE MOVER A RESERVATORIO
    # ===============================================================================
    ...
    # sfs_aux = [sf1_aux, sf2_aux, sf3_aux, sf4_aux, sf5_aux, sf6_aux]
    sfs_original = [sf1, sf2, sf3, sf4, sf5, sf6]
    
    gm.model.occ.synchronize()
    
    # ===============================================================================
    # FIM DAS COPIAS INTERESSANTES PARA O TESTE DE MOVER A RESERVATORIO
    # ===============================================================================
    ...
    surf_boxk = gm.model.occ.addSurfaceLoop([sf1, sf2, sf3, sf4, sf5, sf6]) 
    #surf_boxk = gm.model.occ.addSurfaceLoop(sfs_aux)
    vol_box = gm.model.occ.addVolume([surf_boxk])
    ...
    gm.model.occ.remove([(2, surf_boxk)], recursive=False)
   ...
   # ===============================================================================
   # FROM PIRAMIDAL TO TETRAHEDRAL ELEMENTS
   # ===============================================================================
    types, elemTags, nodeTags = gm.model.mesh.getElements(3)

    # volume onde os novos elementos vão entrar
    entities = gm.model.getEntities(3)
    entityTag = entities[0][1]

    pyramid_tags = []
    newTetNodes = []
    newTetTags = []

    maxTag = gm.model.mesh.getMaxElementTag()
    tagCounter = maxTag + 1

    for t, elems, nodes in zip(types, elemTags, nodeTags):
        
        print(f'\n===================Element reconizing====================\n\n')
        print(f"Element type: {t}, Number of elements: {len(elems)}")

        # 14 = pirâmide linear
        if t == np.int32(7):
            gm.model.addPhysicalGroup(3, [7], tag=100)
            gm.model.setPhysicalName(3, 100, "Pyramids")

            pyramid_tags.extend(elems)

            # cada pirâmide tem 5 nós
            for i in range(0, len(nodes), 5):
                n1, n2, n3, n4, n5 = nodes[i:i+5]

                # escolha de diagonal opcional
                if (n1 + n3) < (n2 + n4):
                    tet1 = [n1, n2, n3, n5]
                    tet2 = [n1, n3, n4, n5]
                else:
                ...
                # salva conectividade
                newTetNodes.extend(tet1)
                newTetNodes.extend(tet2)

                # cria tags únicas
                newTetTags.append(tagCounter)
                tagCounter += 1
                newTetTags.append(tagCounter)
                tagCounter += 1
    ...
    # remove pirâmides
    if pyramid_tags:
        gm.model.mesh.removeElements(3, entityTag, pyramid_tags)

    # adiciona tetraedros
    if newTetTags:
        gm.model.mesh.addElements(
            3,              # dimensão
            entityTag,      # volume geométrico
            [4],            # tipo tetra linear
            [newTetTags],   # ids
            [newTetNodes]   # nós achatados
        )
    gm.model.occ.synchronize()
